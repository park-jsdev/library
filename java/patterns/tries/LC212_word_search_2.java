// Tries - Backtracking with Trie
// Time: O(R x L + R x C x Lmax), Building the Trie is O(W x L) where W is words with avg length L, and R x C are rows and columns
// Space: O(W x L + Lmax + R x C), the Trie has a O(W x L) space, DFS stack has O(Lmax), and the HashSet for visited has
// O(R x C)
/**
    In order to solve this problem, you need to use:
    1. Trie to store and efficiently search the wrods
    2. DFS with Backtracking
    3. visited HashSet for grid traversal
 */
...
class Solution {
    ...
    public void dfs(int r, int c, Trie node, String word, HashSet<String> res, HashSet<String> visit, char[][] board, Trie root){
        ...
        if (r < 0 ||
            ...
            // Perform DFS recursively on 4 directions
