// Heaps - Design
// Time: O(n log n) from getNewsFeed sorting the PQ
// Space: O(n) from the PQ storage
/**
    In essence, we use:
    1. Follow/unfollow: a hashmap of user ids, which maps to a hashset of folowee ids,
    2. Tweets: Hash map of user ids which maps to a list of [count, tweetids],
    3. News Feed: put the time of the tweets in a max heap to manage.
 */

class Twitter {
    int count;
    HashMap<Integer, List<int[]>> tweetMap; // maps to (Int, List)
    HashMap<Integer, HashSet<Integer>> followerMap; // maps to (Int, HashSet)
