...
data <- read_excel("data/data.xlsx", sheet = "Disease vs AI Task")
...
stacked_bar_task <- ggplot(df_for_chord, aes(x = TaskArea, y = Count, fill = DiseaseModel)) +
  ...
  theme(
    ...
    axis.title = element_text(size = 22),  # Increase axis title size
    legend.position = "right",
    legend.title = element_text(size = 20)  # Ensure legend title size is consistent
  ) +
  coord_flip() +
  labs(y = "Count", x = "AI/ML Task Area", fill = "Disease Model")  # Add space in legend title
