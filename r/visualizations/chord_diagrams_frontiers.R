...
# Define the Frontiers color palette
frontiers_colors <- c("#E24E42", "#E9B000", "#EB6E80", "#6EC2EB", "#E7BBE3", "#7BC043", 
                      "#F5A623", "#56C5FF", "#955BA5", "#79BD9A", "#C55A11", "#F98866", 
                      "#FF420E")

# Ensure the color palette is long enough
if (length(frontiers_colors) < length(unique(c(df_for_chord$DiseaseModel, df_for_chord$TaskArea)))) {
  frontiers_colors <- rep(frontiers_colors, length.out = length(unique(c(df_for_chord$DiseaseModel, df_for_chord$TaskArea))))
}

# Create a named vector of colors
category_colors <- frontiers_colors[1:length(unique(c(df_for_chord$DiseaseModel, df_for_chord$TaskArea)))]
...
png("chord_diagram_frontiers.png", width = 2200, height = 2200, bg = "white")  # Adjust canvas size
circos.clear()
circos.par(
...
bar_plot_disease <- ggplot(df_disease, aes(x = reorder(DiseaseModel, Count), y = Count, fill = DiseaseModel)) +
  ...
  scale_fill_manual(values = setNames(frontiers_colors[1:length(unique(df_disease$DiseaseModel))], levels(df_disease$DiseaseModel))) +  # Use the defined colors for the bar plot
...
ggsave("bar_plot_disease_model_frontiers.png", bar_plot_disease, width = 16, height = 12, bg = "white")
...
bar_plot_task <- ggplot(df_task, aes(x = reorder(TaskArea, Count), y = Count, fill = TaskArea)) +
  geom_bar(stat = "identity", color = "black", size = 1, show.legend = FALSE) +  # Add black borders to bars and increase border thickness
  scale_fill_manual(values = setNames(frontiers_colors[1:length(unique(df_task$TaskArea))], levels(df_task$TaskArea))) +  # Use the defined colors for the bar plot
  theme_minimal(base_size = 25) +  # Increase base font size further
  theme(
    panel.background = element_rect(fill = "white", color = NA),  # White background
    plot.background = element_rect(fill = "white", color = NA),   # White background for entire plot
...
ggsave("bar_plot_task_area_frontiers.png", bar_plot_task, width = 16, height = 12, bg = "white")
