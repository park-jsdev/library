...
# Define the Lancet color palette
lancet_colors <- c("#00468B", "#ED0000", "#42B540", "#0099B4", "#925E9F", "#FDAF91", 
                   "#AD002A", "#ADB6B6", "#1B1919", "#DC0000", "#FF61A6", "#7E6148", 
                   "#A58400")

# Ensure the color palette is long enough
if (length(lancet_colors) < length(unique(c(df_for_chord$DiseaseModel, df_for_chord$TaskArea)))) {
  lancet_colors <- rep(lancet_colors, length.out = length(unique(c(df_for_chord$DiseaseModel, df_for_chord$TaskArea))))
}

# Create a named vector of colors
category_colors <- lancet_colors[1:length(unique(c(df_for_chord$DiseaseModel, df_for_chord$TaskArea)))]
...
png("chord_diagram_lancet.png", width = 2200, height = 2200, bg = "white")  # Adjust canvas size
circos.clear()
circos.par(
...
bar_plot_disease <- ggplot(df_disease, aes(x = reorder(DiseaseModel, Count), y = Count, fill = DiseaseModel)) +
  ...
  scale_fill_manual(values = setNames(lancet_colors[1:length(unique(df_disease$DiseaseModel))], levels(df_disease$DiseaseModel))) +  # Use the defined colors for the bar plot
...
ggsave("bar_plot_disease_model_lancet.png", bar_plot_disease, width = 16, height = 12, bg = "white")
...
bar_plot_task <- ggplot(df_task, aes(x = reorder(TaskArea, Count), y = Count, fill = TaskArea)) +
  geom_bar(stat = "identity", color = "black", size = 1, show.legend = FALSE) +  # Add black borders to bars and increase border thickness
  scale_fill_manual(values = setNames(lancet_colors[1:length(unique(df_task$TaskArea))], levels(df_task$TaskArea))) +  # Use the defined colors for the bar plot
  theme_minimal(base_size = 25) +  # Increase base font size further
  theme(
    panel.background = element_rect(fill = "white", color = NA),  # White background
    plot.background = element_rect(fill = "white", color = NA),   # White background for entire plot
...
ggsave("bar_plot_task_area_lancet.png", bar_plot_task, width = 16, height = 12, bg = "white")
