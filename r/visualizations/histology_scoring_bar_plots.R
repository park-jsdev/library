...
# Create the bar plot for Scoring System Class
bar_plot_scoring <- ggplot(counts_scoring, aes(x = reorder(wrap_labels(`Scoring System Class`), -n), y = n, fill = `Scoring System Class`)) +
  ...
  labs(y = "Count", x = "Scoring System Class")
...
# Create the bar plot for Histological Evidence Category
bar_plot_evidence <- ggplot(counts_evidence, aes(x = reorder(wrap_labels(`Histological Evidence Category`), -n), y = n, fill = `Histological Evidence Category`)) +
  geom_bar(stat = "identity", color = "black", size = 2, width = 0.3) +  # Further reduce bar width for more space
  scale_fill_manual(values = custom_palette) +  # Use the custom color palette
  theme_minimal(base_size = 20) +  # Increase base font size
  theme(
    ...
    plot.background = element_rect(fill = "white", color = NA),   # White background for entire plot
    panel.grid = element_blank(),  # Remove grid lines
    axis.text.x = element_text(size = 20),  # Horizontal axis text
    axis.text.y = element_text(size = 20),  # Increase axis text size
    axis.title.x = element_text(size = 22, margin = margin(t = 20, r = 20, b = 50, l = 20)),  # Increase axis title size and margin
    axis.title.y = element_text(size = 22, margin = margin(t = 20, r = 20, b = 20, l = 20)),  # Increase axis title size and margin
    legend.position = "none"  # Remove legend
  ) +
  labs(y = "Count", x = "Histological Evidence Category")
