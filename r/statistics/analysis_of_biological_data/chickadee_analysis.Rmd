...
# Use a table to display categorical variable
...
# Bar charts for large counts

# Histograms for phenotypes

# Use other plots two compare 2 variables

# Use line graphs for comparing 2 continuous variables


# Bar Charts - unfactored
ggplot(data = chickadeeData, aes(x = EscShi)) +
geom_bar(stat = "count") +
labs(x = "Pathogen Abundance", y = "Frequency") +
...
geom_bar(stat = "count") +
labs(x = "Pathogen Diversity Categorical", y = "Frequency") +
theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
labs(x = "Pathogen Diversity", y = "Frequency")

# Contingency Table - EscShi and Site
chickadeeEscShiTable <- table (chickadeeData$EscShi, chickadeeData$Habitat)
chickadeeEscShiTable
addmargins(chickadeeEscShiTable, margin = c(1,2), FUN = sum, quiet = TRUE)

# Grouped Bar Graph
ggplot(chickadeeData, aes(x = Habitat, fill = EscShi)) +
geom_bar(stat = "count", position = position_dodge2(preserve="single")) +
labs(x = "Habitat", y = "EscShi Abundance")

# Mosaic Plot
mosaicplot(t(chickadeeEscShiTable), sub = "Habitat",
ylab = "EscShi Abundance", main = "")

# Scatter Plot
ggplot(data = chickadeeData,
aes(x = Habitat, y = EscShi)) +
geom_point(size = 2) +
labs(x = "Habitat", y = "EscShi Abundance")
